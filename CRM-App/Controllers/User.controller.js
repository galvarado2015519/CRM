...
function saveUser(req, res) {
    ...
    User.findOne({$or:[{email: body.email},{username: body.username}]},(err, findUserRepeat)=>{
        if(err){
            ...
            res.status(200).send({message:"Este correo o nombre de usuario ya estan registrados, prueba con otro"});
        }else{
            user.name = body.name;
                ...
                if(err){
                    ...
                    user.save((err, userSave)=>{
                        if(err){    
                            ...
                            res.status(200).send({UsuarioCreado: userSave, token: jwt.createToken(userSave)});
            ...
            if(err){
                ...
                bcrypt.compare(body.password, userFind.password, (err, passwordCheck)=>{
                    if(err){
                        ...
                        res.send({user: userFind, token: jwt.createToken(userFind)});
                    }else{
                        res.status(200).send({message:"La contraseña es inconrrecta"});
                    }
                })
            }else{
                res.status(200).send({mesagge: "Este usuario no existente"});
            }
        });
    }else{
