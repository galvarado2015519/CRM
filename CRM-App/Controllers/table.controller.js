...
function viewTables(req, res) {
    ...
    User.findById(req.user.sub,(err, userFind)=>{
        if(err){
            ...
            Table.find({_id: userFind.table})
            .populate('task').exec((err, tables)=>{
                if(err){
                    res.status(500).send({message:"Error general del servidor ", err});
                }else if(tables){
                    res.send({table:tables});
                }else{
                    res.status(400).send({message:"Error inesperado al listar las tablas"});
                }
            })
        }else{
            res.status(400).send({message:"El usuario no esta logeado, no middleware"});
        }
    })
}
